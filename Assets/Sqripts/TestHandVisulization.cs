...
public class TestHandVisulization : MonoBehaviour
{
	...
	{
		// Перевіряємо, чи об'єкт для збереження існує
		if (objectToSave != null)
		{
			// Вказуємо шлях, де зберегти префаб
			string prefabPath = "Assets/Prefabs/" + GestureName + ".prefab";

			// Викликаємо метод для створення префабу
			PrefabUtility.SaveAsPrefabAsset(objectToSave, prefabPath);

			Debug.Log("Префаб " + objectToSave.name + " збережено за шляхом: " + prefabPath);
		}
		else
		{
			Debug.LogError("Об'єкт для збереження префабу null.");
		}
	}
